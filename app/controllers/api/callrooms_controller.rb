class Api::CallroomsController < ApplicationController
  ...
  def new
    ...
    # if user_signed_in?
    #   @callroom10 = Callroom.find_by(student_id: current_user.id)
    #   if session[:callroom].present? && session[:callroom]["status"] == 2 && @callroom10.nil?
